$(document).ready(function () {
  var dataTable = $("#tabel-penjaminan-mutu").DataTable({
    ...
    ajax: {
      url: baseURL + "beranda/Penjaminan_mutu_30/data_penjaminan_mutu",
      type: "POST",
      data: function (d) {
  ...
  $("#tabel-riwayat-penjaminan-mutu_filter")
    .detach()
  ...
  $("#kode_pt_riwayat")
    ...
    .on("change", function (e) {
      ...
      $.ajax({
        url: baseURL + "get-data-pt-30", // Endpoint untuk mendapatkan data
        type: "POST",
        data: {
          ...
          tooltip: {
            callbacks: {
              label: function (context) {
                ...
                let currentTipologi =
                  context.chart.data.datasets[0].tipologi[index]; // Ambil tipologi dari data
  ...
  function panRight(data) {
    if (xMax + panStep < fullData.length) {
      // Cek apakah masih bisa ke kanan
      xMin += panStep;
      xMax += panStep;
      updateChart(data);
    }
...
$("#form-import-data").on("submit", function (event) {
  ...
  Swal.fire({
    title: "Import Data?",
    text:
      "Apakah anda yakin ingin import data pada " +
      periode.substr(0, 4) +
      prd +
      "?",
    icon: "warning",
