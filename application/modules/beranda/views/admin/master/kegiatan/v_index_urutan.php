            ...
            <div class="card-content">
              <div class="card-body">
                <div id="table-kegiatan-container">
                  <?php $this->load->view('admin/master/kegiatan/_table'); ?>
                </div>
              </div>
            </div>
          </div>
...
<script type="text/javascript">
  ...
  $(document).on('change', '.ubah-urutan', function() {
    ...
    let urutanLama = parseInt(select.data('old-value'));
  ...
  function reloadTableKegiatan(callback = null) {

    // simpan page & search
    let currentPage = 0;
    let currentSearch = '';

    if ($.fn.DataTable.isDataTable('#tabel-kegiatan')) {

      let table = $('#tabel-kegiatan').DataTable();
    ...
    $("#table-kegiatan-container").load(
      baseURL + "admin/kelola-kegiatan/table",
      function() {
